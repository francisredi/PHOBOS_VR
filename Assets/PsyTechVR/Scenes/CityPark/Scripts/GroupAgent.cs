...
namespace CrowdSim
{
	...
	static class Constants
	{
		...
		public const int TAG_ALONG = 9000;
		public const int PASSING_GROUP = 9001;
		public const int NORMAL = 9002;
		public const int HEAD_ON_COLLISION_AVOIDANCE = 9003;
		public const int REAR_END_COLLISION_AVOIDANCE = 9004;
		public const int SIDE_COLLISION_AVOIDANCE = 9005;
		public const int URGENT_COLLISION_AVOIDANCE = 9006;
		public const int INITIAL_POSITIONING = 9007;
		public const float INTIMATE_DISTANCE = 6.0f; // 6.0 m away
		public const float MAX_CONSIDERATION_DISTANCE = 15.0f;
		public const float TAU_FOR_BEING_COLLINEAR = 25.0f;
		
		public const int RIGHT_COLLISION = 9008;
		public const int LEFT_COLLISION = 9009;
		public const int INTO_COLLISION = 9010;
		public const int LEFT = 6000;
		public const int RIGHT = 6001;
		
		public const float K = 1.4f;
	}
	#endregion

	public class GroupAgent : StateMachineBehaviourEx {
		...
		private void ReassignSlots(){
			...
			for(int i = 0; i < membersList.Count; i++){
				...
				member.GetComponent<NavigateCity>().assignedSlotNumber = slotNum;
			}
		...
		public bool AddMember(GameObject pedestrianObject, int slotTarget){
			if(!membersList.Contains(pedestrianObject)){
				...
				if(slotNum > 0){
					setSlotAvailable(slotNum,false);
					numMembers++;
					pedestrianObject.GetComponent<NavigateCity>().assignedGA = this.gameObject;
					pedestrianObject.GetComponent<NavigateCity>().assignedSlotNumber = slotNum;
					pedestrianObject.GetComponent<NavigateCity>().currentState = CrowdSim.NavigateCity.NPCStates.MovingWithGroup;
		...
		void Start () {
			...
			originalSpeed = 1.0f;
			scale = CitySimulator.scale;
			radius = 1.2f;

			float render = 0.0f;
			if(CitySimulator.renderGroupAgent) render = 1.0f;
			...
			graph = sim.GetComponent<CitySimulator>().GetGlobalPathPlanningGraph();
			destinationList = sim.GetComponent<CitySimulator>().GetDestinationList();

			groupAgentRef = sim.GetComponent<CitySimulator>().GetGroupAgents(); // added 12/2/2014
		...
		void WaitForLoadFinish_FixedUpdate()
		{
			...
			if(CitySimulator.usingGroupGMAbasedCA){
		...
		void UpdateOffSetWaypointTransform(){
			...
			if(currentWP == graph.getPathLength(pathList)){ // if reached end, there is no next node
				return;
			}
			...
			if(CitySimulator.offsetEnabled){
		...
		void Moving_FixedUpdate()
		{
			...
			if(currentWP < graph.getPathLength(pathList)){
				...
				if(Physics.Raycast(boundaryPointHelper.transform.position,Vector3.down,out hit,5.0f, CitySimulator.m_FloorLayerForRayCast.value)){ // ray cast downward
					transform.position = new Vector3(transform.position.x,hit.point.y+0.3f,transform.position.z);
				}

				if(CitySimulator.usingGroupGMAbasedCA){ // GMA-based collision avoidance
					NavigateByGMA();
				}
				else{ // AMVR case!

					if(nextNextNode == null){ // path ends, so keep going regardless
						...
						if(stat1!=null && stat2!=null){ // prepare to pass crosswalk
							//targetSpeed = originalSpeed;
						...
						else if(stat1!=null && stat2==null){ // crossing crosswalk
							if(prepared && ((Time.time - startTime) > 5.0f)){ // enter once, only if prepared for crosswalk
								prepared = false;
								//print ("crosswalk");
						...
						else{ // path is not through crosswalk, so continue
							targetSpeed = originalSpeed; // go back to normal speed if not already
						}
					}
		...
		void Waiting_FixedUpdate()
		{
			if(stat1.WaitAtCrossWalk()){
				...
				if(Vector3.Distance(prevOffsetWaypoint.transform.position,transform.position) < 0.5f){ // if near, stop
			...
			else{ // allowed to pass street, so continue
				flagWantToGo = true;

				if(waitAtTrafficLight && CheckIfReadyToGo()){
					for(int i = 0; i < membersList.Count; i++){
						GameObject member = (GameObject) membersList[i];
						member.GetComponent<NavigateCity>().MoveWithGroup();
					}
					waitAtTrafficLight = false;
					Return();
					//return;
		...
		private bool CanSeeObject(Transform seer, Transform target, float seeRange, float sightAngle, out float distance)
		{
			...
			if (Physics.Raycast (seeObject,out hit,distance, CitySimulator.m_WallLayerForRayCast.value)
			    || Physics.Raycast (seeObject,out hit,distance, CitySimulator.m_VehicleLayerForRayCast.value)){ // ray cast to other group
				return false;
			}
		...
		private void ComputeNearestGroupAgent(float maxCollisionAvoidanceDistance){
			...
			for (int i = 0; i < groupAgentRef.Length; i++) {
				...
				if(CanSeeObject(this.boundaryPointHelper.transform,go.GetComponent<GroupAgent>().boundaryPointHelper.transform,maxCollisionAvoidanceDistance,80.0f,out distance)) // can see other group agent, 90, 135
				{
		...
		private void UpdateGMATimeStep(){ // do once per second
			...
			if(nearestGroupAgent != null){
				...
				GroupAgent nearestGroupAgentWaypointAgent = nearestGroupAgent.gameObject.GetComponent<GroupAgent>();
				
				if(positions.Count == GMASteps && nearestGroupAgentWaypointAgent.positions.Count == GMASteps){ // check GMA for specified time steps
					...
					for(int i = 1; i < GMASteps; i++){
						...
						if(((newDistance/prevDistance) < 1.0f) && (Mathf.Abs(Mathf.Abs(gazeMovementAngle2)-Mathf.Abs (gazeMovementAngle1)) < threshold)){ // consistent GMA AND decreasing distance?
							if(i == GMASteps-1){
		...
		private void SteerForTargetPosition(){
			...
			if(caState == Constants.HEAD_ON_COLLISION_AVOIDANCE){ // shift formation left or right, not turn!
				...
				if(angle < 10.0f){ // side step only, already absolute angle
					sideStepping = true;
				...
				else{ // moving direction has same value as target direction (rotate)
				...
				if(angle < 10.0f){ // side step, already absolute angle
					sideStepping = true;
					
					if(collisionDirection == Constants.RIGHT_COLLISION){ // go left
				...
				if(caState != Constants.INITIAL_POSITIONING && Mathf.Abs(angle) < 10.0f){ // side step
					sideStepping = true;
					
					if(angle == 0){ // go straight
						transform.Translate(0.0f,0.0f,Time.deltaTime * gSpeed);
					}
					else if(angle < 0){ // go left
		...
		private void HandleHeadCollision(){
			...
			if(gazeMovementAngle1 < 0.0f && observedGazeMovementAngle1 < 0.0f){ // go right
				collisionDirection = Constants.LEFT_COLLISION;
			}
			else if(gazeMovementAngle1 > 0.0f && observedGazeMovementAngle1 > 0.0f){ // go left
		...
		private void HandleRearEndCollision(){
			...
			if(gSpeed > nearestGroupAgent.GetComponent<GroupAgent>().gSpeed){ // overtake if this group agent is faster than the observed
				...
				if(gazeMovementAngle1 < 0.0f){ // go right
					collisionDirection = Constants.LEFT_COLLISION;
				}
				else{ // go left
					collisionDirection = Constants.RIGHT_COLLISION;
		...
		private void HandleSideCollision(){
			...
			if(Mathf.Abs(gazeMovementAngle1) > Mathf.Abs(observedGazeMovementAngle1)){ // pass the other by speeding up
			...
			else{ // slow down depending on distance
		...
		private void NavigateByGMA(){
			...
			if(nearestGroupAgent != null){ // there is a collidee, so set up for collision avoidance
				SetupCollisionCases();
			}
			else if(caState == Constants.PASSING_GROUP){ // overtaking mode
				//gaSpot.renderer.material.color = Color.black;
				
				if(timeInCurrentState - overtakeStartTime > 3.0f){ // after 3 seconds, back to normal mode
					caState = Constants.NORMAL;
			...
			if(caState == Constants.REAR_END_COLLISION_AVOIDANCE){
				...
				if(distanceCA < 0.1f){ // reached CA target
					caState = Constants.PASSING_GROUP;
					overtakeStartTime = timeInCurrentState;
				}
				if(distanceDest < distanceCA || ang > 30.0f){ // closer to destination now? or angle not proper
					caState = Constants.NORMAL;
				}
			}
		...
		private bool HoldHands(GameObject m1, GameObject m2){
			...
			if(oppositeGender && Vector3.Distance(m1.transform.position,m2.transform.position) < 0.7f &&
			   angle < 100.0f && angle > 80.0f ){ // if they are near enough and side-by-side, hold hands
				...
				m1.GetComponent<NavigateCity>().SetRightHand(holdPosition,true);
				m2.GetComponent<NavigateCity>().SetLeftHand(holdPosition,true);
				return true; // success
		...
		private void SetUpGestures(int leftSide, int rightside){
			...
			switch(rand){
				...
				case 2:*/ // on shoulder
					...
					//break;
				/*	m1.GetComponent<NavigateCity>().currentGesture = NavigateCity.GestureStates.HandOnShoulder;
					break;
				case 3:*/ // upper back
					//m2.GetComponent<NavigateCity>().currentGesture = NavigateCity.GestureStates.HandOnShoulder;
					...
					/*break;
				case 4:
					m1.GetComponent<NavigateCity>().currentGesture = NavigateCity.GestureStates.HandOnUpperBack;*/
			...
			/*
					break;
				case 5:
					m2.GetComponent<NavigateCity>().currentGesture = NavigateCity.GestureStates.HandOnUpperBack;
					break;*/
			//}
		}
		...
		private void ApplyIntimateGesture(int leftSide, int rightside){
			...
			for(int i = 0; i < membersList.Count; i++){
				...
				if(go.GetComponent<NavigateCity>().assignedSlotNumber == leftSide){
					m1 = go;
					leftSideGestureState = getGesture(leftSide);
				}
				else if(go.GetComponent<NavigateCity>().assignedSlotNumber == rightside){
					m2 = go;
			...
			// reset
			m1.GetComponent<NavigateCity>().SetRightHand(Vector3.zero,false);
			m1.GetComponent<NavigateCity>().SetLeftHand(Vector3.zero,false);
			m2.GetComponent<NavigateCity>().SetRightHand(Vector3.zero,false);
			m2.GetComponent<NavigateCity>().SetLeftHand(Vector3.zero,false);
			...
			if ((leftSideGestureState == GestureStates.HoldingHand) && (rightSideGestureState == GestureStates.HoldingHand)) {
				if(!HoldHands (m1, m2)){ // if not successful
			...
			// shoulder
				...
				if(Vector3.Distance(m1.transform.position,m2.transform.position) < 0.7f &&
				   Vector3.Angle(m2.transform.position-m1.transform.position,m1.transform.forward) < 80.0f ){ // if they are near enough and other in front
					Vector3 pos = m2.GetComponent<NavigateCity>().GetLeftShoulder();
					//pos.x = pos.x + 0.05f;
					m1.GetComponent<NavigateCity>().SetRightHand(pos,true);
				}
			}
			else if (rightSideGestureState == GestureStates.HandOnShoulder) {
				if(Vector3.Distance(m1.transform.position,m2.transform.position) < 0.7f &&
				   Vector3.Angle(m1.transform.position-m2.transform.position,m2.transform.forward) < 80.0f ){ // if they are near enough and other in front
					Vector3 pos = m1.GetComponent<NavigateCity>().GetRightShoulder();
					//pos.x = pos.x - 0.05f;
					m2.GetComponent<NavigateCity>().SetLeftHand(pos,true);
				}
			}
			// upper back
				...
				if(Vector3.Distance(m1.transform.position,m2.transform.position) < 0.7f &&
				   Vector3.Angle(m2.transform.position-m1.transform.position,m1.transform.forward) < 80.0f ){ // if they are near enough and other in front
					Vector3 pos = m2.GetComponent<NavigateCity>().GetUpperBack();
					pos.x = pos.x - 0.05f;
					m1.GetComponent<NavigateCity>().SetRightHand(pos,true);
				}
			}
				...
				if(Vector3.Distance(m1.transform.position,m2.transform.position) < 0.7f &&
				   Vector3.Angle(m1.transform.position-m2.transform.position,m2.transform.forward) < 80.0f ){ // if they are near enough and other in front
					Vector3 pos = m1.GetComponent<NavigateCity>().GetUpperBack();
					pos.x = pos.x - 0.05f;
					m2.GetComponent<NavigateCity>().SetLeftHand(pos,true);
				}
			}
			// lower back
				...
				if(Vector3.Distance(m1.transform.position,m2.transform.position) < 0.7f &&
				   Vector3.Angle(m2.transform.position-m1.transform.position,m1.transform.forward) < 80.0f ){ // if they are near enough and other in front
					Vector3 pos = m2.GetComponent<NavigateCity>().GetLowerSpine();
					pos.z = pos.z - 0.05f;
					m1.GetComponent<NavigateCity>().SetRightHand(pos,true);
				}
			}
				...
				if(Vector3.Distance(m1.transform.position,m2.transform.position) < 0.7f &&
				   Vector3.Angle(m1.transform.position-m2.transform.position,m2.transform.forward) < 80.0f ){ // if they are near enough and other in front
					Vector3 pos = m1.GetComponent<NavigateCity>().GetLowerSpine();
					pos.z = pos.z - 0.05f;
					m2.GetComponent<NavigateCity>().SetLeftHand(pos,true);
				}
			}
		...
		float ForwardCast(ref float tempDistance){
			...
			if (!slotIsAvailable(5) && Physics.Raycast (rayS5,out hit,10.0f,CitySimulator.raycastLayers.value)){ // ray cast to destination
			...
			if (!slotIsAvailable(4) && Physics.Raycast (rayS4,out hit,10.0f,CitySimulator.raycastLayers.value)){ // ray cast to destination
				if(hit.collider.gameObject.tag == "Pedestrian" && RelativeDirection(hit.collider.gameObject) == Constants.TAIL){
					// ignore pedestrian since coming up from behind and pedestrian moving forward
				}
				else{
			...
			if (!slotIsAvailable(3) && Physics.Raycast (rayS3,out hit,10.0f,CitySimulator.raycastLayers.value)){ // ray cast to destination
				if(hit.collider.gameObject.tag == "Pedestrian" && RelativeDirection(hit.collider.gameObject) == Constants.TAIL){
					// ignore pedestrian since coming up from behind and pedestrian moving forward
				}
				else{
			...
			if (!slotIsAvailable(2) && Physics.Raycast (rayS2,out hit,10.0f,CitySimulator.raycastLayers.value)){ // ray cast to destination
				if(hit.collider.gameObject.tag == "Pedestrian" && RelativeDirection(hit.collider.gameObject) == Constants.TAIL){
					// ignore pedestrian since coming up from behind and pedestrian moving forward
				}
				else{
			...
			if (!slotIsAvailable(1) && Physics.Raycast (rayS1,out hit,10.0f,CitySimulator.raycastLayers.value)){ // ray cast to destination
				if(hit.collider.gameObject.tag == "Pedestrian" && RelativeDirection(hit.collider.gameObject) == Constants.TAIL){
					// ignore pedestrian since coming up from behind and pedestrian moving forward
				}
				else{
		...
		float BoundaryCast(ref float tempDistance){
			...
			//if(!slotIsAvailable(5) && !slotIsAvailable(3)){
				...
				if (Physics.Raycast (rayL,out hit,5.0f, CitySimulator.m_WallLayerForRayCast.value)
				    /*|| Physics.Raycast (rayL,out hit,5.0f, PedestrianSimulator.m_VehicleLayerForRayCast.value)*/){ // ray cast to destination
					if(hit.distance < tempDistance) tempDistance = hit.distance;
			...
			if (Physics.Raycast (rayLF,out hit,5.0f, CitySimulator.m_WallLayerForRayCast.value)
			    /* || Physics.Raycast (rayLF,out hit,5.0f, PedestrianSimulator.m_VehicleLayerForRayCast.value)*/){ // ray cast to destination
				if(hit.distance < tempDistance) tempDistance = hit.distance;
				Debug.DrawLine(boundaryPointHelper.transform.position,hit.point,groupColor);
				lockS3S1 = false;
				lockS5S3 = false;
			...
			//if (!slotIsAvailable (2) && !slotIsAvailable (4)) {
				if (Physics.Raycast (rayR, out hit, 5.0f, CitySimulator.m_WallLayerForRayCast.value)
					/*|| Physics.Raycast (rayR,out hit,5.0f, PedestrianSimulator.m_VehicleLayerForRayCast.value)*/) { // ray cast to destination
					if (hit.distance < tempDistance) tempDistance = hit.distance;
					Debug.DrawLine (boundaryPointHelper.transform.position, hit.point, groupColor);
					lockS2S1 = false;
					lockS4S2 = false;
			...
			if (Physics.Raycast (rayRF, out hit, 5.0f, CitySimulator.m_WallLayerForRayCast.value)
			    /*|| Physics.Raycast (rayRF,out hit,5.0f, PedestrianSimulator.m_VehicleLayerForRayCast.value)*/) { // ray cast to destination
				if (hit.distance < tempDistance) tempDistance = hit.distance;
				Debug.DrawLine (boundaryPointHelper.transform.position, hit.point, groupColor);
				lockS2S1 = false;
				lockS4S2 = false;
		...
		private bool CheckIfReadyToGo(){
			for(int i = 0; i < membersList.Count; i++){
				GameObject member = (GameObject) membersList[i];
				if(!member.GetComponent<NavigateCity>().PersonIsReadyToGo()) return false; // at least one not ready
			}
			return true;
		}
		...
		void Huddle_FixedUpdate()
		{
			...
			if(timeInCurrentState > startMovingSec){  // if time in state more than required wait time
				flagWantToGo = true;
			}
		...
		private void AdjustWalkingFormation(){
			...
			if(distanceToBoundary < 1.0f || comingDistance < 5.0f){ // contract to river
				if(formationState != 2) s = 0.0f;
				formationState = 2;
			}
			else if(distanceToBoundary < 2.0f || comingDistance < 10.0f){ // contract to wide U
				if(formationState != 1) s = 0.0f;
				formationState = 1;
			}
			else{ // expand to abreast
