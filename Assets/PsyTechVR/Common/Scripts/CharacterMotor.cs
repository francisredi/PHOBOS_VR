...
public class CharacterMotor : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        tr = transform;
        distToGround = tr.GetComponent<CharacterController>().bounds.extents.y;
    }
    ...
    private void UpdateFunction()
    {
		...
		///////////////////
		// we should move this into a separate script...
		Vector3 stepVel = velocity;
		stepVel.y = 0;
		if( !stepVel.Equals(Vector3.zero) && grounded)
		{
	...
	{
		if(other.gameObject.layer == LayerMask.NameToLayer("NPC_Cars_Collision") ) // it is a car, so slide against it
		{
			Vector3 slideVec = (transform.position - other.transform.position);
			slideVec.y = 0;
			slideVec.Normalize();
			transform.Translate( slideVec * 0.01f, Space.World );
			SetControllable( false );
		}
		else if(other.gameObject.layer == LayerMask.NameToLayer("NPC_People") ) // it is an npc, so ... slide against it for now
		{
			Vector3 slideVec = (transform.position - other.transform.position);
			slideVec.y = 0;
			slideVec.Normalize();
			transform.Translate( slideVec * 0.02f, Space.World );
			SetControllable( false );
		}
	}
    ...
    {
        return Physics.Raycast(transform.position, -Vector3.up, distToGround + 0.1f); // new 5/29/2015
    ...
    void SetControllable(bool controllable)
    {
		...
		SendMessage("SetHaltUpdateMovement", !controllable, SendMessageOptions.DontRequireReceiver); // if we've a OVR player controller, also pass in the state.
    }
