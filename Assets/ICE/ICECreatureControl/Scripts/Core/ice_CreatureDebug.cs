...
namespace ICE.Creatures.Objects
{
	...
	public class GizmoObject : System.Object
	{
		...
		public void DrawPatrol()
		{
			...
			if( _last_waypoint_target != null )
			{
				...
				_last_target_stop_distance = _last_waypoint_target.Move.StopDistance;
			}
			...
			for(  int i = 0 ; i < CreatureControl.Creature.Missions.Patrol.Waypoints.Waypoints.Count ; i++ )
			{
				...
				_target_stop_distance = _target.Move.StopDistance;
			...
			for(  int i = 0 ; i < CreatureControl.Creature.Interaction.Interactors.Count ; i++ )
			{
				...
				if( _target_game_objects != null && _target_game_objects.Count > 0 )
				{
					foreach( GameObject _target_game_object in _target_game_objects )
					{
						...
						_target.UpdateOffset( _interactor.Move.Offset );
						_target.Move.StopDistance = _interactor.Move.StopDistance;
						_target.Move.StopDistanceZoneRestricted = _interactor.Move.StopDistanceZoneRestricted;
						_target.Move.RandomRange = _interactor.Move.RandomRange;
						_target.Move.SmoothingMultiplier = 0;//_interactor.DefaultSmoothingMultiplier; // TODO:REWORK TargetSmoothingMultiplier
						_target.Move.IgnoreLevelDifference = _interactor.Move.IgnoreLevelDifference;

						DrawTargetGizmos( _target );
			
						// DIRECTION
						CustomGizmos.Arrow( _target.TargetPosition, _target.TargetDirection * _target.Move.StopDistance , 0.5f, 20 );
					
						Vector3 _last_move_position = _target.TargetMovePosition;
						float _last_stop_distance = _target.Move.StopDistance;
						...
						foreach( InteractorRuleObject _rule in _interactor.Rules )
						{
							...
							{
								_target.UpdateOffset( _rule.Move.Offset );
								_target.Move.StopDistance = _rule.Move.StopDistance;
								_target.Move.RandomRange = _rule.Move.RandomRange;
								_target.Move.SmoothingMultiplier = 0;
								_target.Move.IgnoreLevelDifference = _rule.Move.IgnoreLevelDifference;
							}
							...
							if( _last_move_position != Vector3.zero )
								CustomGizmos.OffsetPath( _last_move_position, _last_stop_distance, _move_position, _target.Move.StopDistance, true );
			
							_last_stop_distance = _target.Move.StopDistance;
	...
	/*	private List<GameObject> GetTargets( TargetObject _target )
		...
		private void DrawDirectionAngle( Transform _center, float _radius, float _angle, float _max = 0, float _min = 0 )
		{
			...
			Vector3 _left_pos = GraphicTools.GetDirectionPosition( _center, - _angle, _radius );
			Vector3 _right_pos = GraphicTools.GetDirectionPosition( _center, _angle, _radius );		
		...
		private void DrawDirectionAngle( Transform _center, float _inner_radius, float _outer_radius, float _outer_radius_max, float _outer_radius_min, float _inner_angle, float _outer_angle )
		{
			...
			Vector3 _left_inner_pos = GraphicTools.GetDirectionPosition( _center, - _inner_angle, _inner_radius );
			Vector3 _right_inner_pos = GraphicTools.GetDirectionPosition( _center, _inner_angle, _inner_radius );				
			Vector3 _left_outer_pos = GraphicTools.GetDirectionPosition( _center, - _outer_angle, _outer_radius );
			Vector3 _right_outer_pos = GraphicTools.GetDirectionPosition( _center, _outer_angle, _outer_radius );
		...
		private void DrawBehaviourModeRuleGizmos( TargetObject _target, BehaviourModeRuleObject _rule )
		{
			...
			float _target_stop_distance = _target.Move.StopDistance;
			...
			else if( _rule.Move.Type == MoveType.ESCAPE )
			{
				...
				Vector3 _left_inner_pos = GraphicTools.GetDirectionPosition( _target.TargetGameObject.transform, - _inner_angle, _inner_radius );
				Vector3 _right_inner_pos = GraphicTools.GetDirectionPosition( _target.TargetGameObject.transform, _inner_angle, _inner_radius );				
				Vector3 _left_outer_pos = GraphicTools.GetDirectionPosition( _target.TargetGameObject.transform, - _outer_angle, _outer_radius );
				Vector3 _right_outer_pos = GraphicTools.GetDirectionPosition( _target.TargetGameObject.transform, _outer_angle, _outer_radius );
				...
				Vector3 _center_pos = GraphicTools.GetDirectionPosition( _target_transform, 0, _inner_radius + ( _inner_radius / 10 ) );
				Vector3 _left_pos = GraphicTools.GetDirectionPosition( _target_transform, - _inner_angle, _inner_radius );
				Vector3 _right_pos = GraphicTools.GetDirectionPosition( _target_transform, _inner_angle , _inner_radius );
		...
		public void DrawPathGizmos()
		{
			...
			if( m_creature_control.Creature.Move.NavMeshAgentReady )
			{
				...
				UnityEngine.AI.NavMeshPath _path = m_creature_control.Creature.Move.NavMeshAgentComponent.path;
		...
		public void DrawOwnerGizmos()
		{
			...
			if( m_creature_control.Creature.Move.FieldOfView > 0 )
			{
				...
				if( _angle < 180 )
				{
					Vector3 _left_pos = GraphicTools.GetDirectionPosition( m_Owner.transform, - _angle, _distance );
					Vector3 _right_pos = GraphicTools.GetDirectionPosition( m_Owner.transform, _angle, _distance );
			...
			if( m_creature_control.Creature.Move.DefaultBody.Type == GroundOrientationType.DEFAULT || m_creature_control.Creature.Move.DefaultBody.Type == GroundOrientationType.QUADRUPED )
			{
				if( m_creature_control.Creature.Move.DefaultBody.UseAdvanced )
				{
					if( ! Application.isPlaying )
					{
						m_creature_control.Creature.Move.CurrentBody = m_creature_control.Creature.Move.DefaultBody;
						m_creature_control.Creature.Move.CurrentBody.Type = GroundOrientationType.QUADRUPED;
						m_creature_control.Creature.Move.Init( m_Owner );
						m_creature_control.Creature.Move.HandleGroundOrientation( m_Owner.transform.rotation );
					}
		...
		public void DrawMoveGizmos()
		{
			...
			float _move_lateral_variance = m_creature_control.Creature.Move.CurrentMove.MoveDeviationVariance;
			...
			if( m_creature_control.Creature.Move.CurrentMove.Type != MoveType.ESCAPE )
				CustomGizmos.OffsetPath( _move_position, _move_stop_distance , _target_move_position , _target.Move.StopDistance );  // PATH NEXT MOVE POSITION TO TARGET MOVE POSITION
		...
		{ 
			...
			//if( _target_selection_angle > 0 && _target_selection_angle <= 180 )
			{
				...
				if( _target_selection_angle < 180 )
				{
					Transform _target_transform = _target.TargetGameObject.transform;
					Vector3 _center = _target_transform.position;
					Vector3 _center_pos = GraphicTools.GetDirectionPosition( _target_transform, 0, _target_selection_distance + ( _target_selection_distance / 10 ) );
					Vector3 _left_pos = GraphicTools.GetDirectionPosition( _target_transform, - _target_selection_angle, _target_selection_distance - 0.25f );
					Vector3 _right_pos = GraphicTools.GetDirectionPosition( _target_transform, _target_selection_angle , _target_selection_distance - 0.25f );
			...
			if( _target.Move.IgnoreLevelDifference )
				CustomGizmos.Circle( _target_pos_move, _target.Move.StopDistance, 5, false ); // IGNORES LEVEL DIFFERNCE SO WE DRAW JUST A CIRCLE
			else
				Gizmos.DrawWireSphere( _target_pos_move, _target.Move.StopDistance );// IF THE LEVEL IS IMPORTANT FOR THE STOP DISTANCE WE DRAW A SPHERE
			// TARGET STOP DISTANCE END

			// MAX AND RANDOM RANGE BEGIN
			CustomGizmos.Circle( _target_pos_offset, _target.Move.RandomRange , 5, false ); // RANDOM RANGE
			CustomGizmos.Circle( _target_pos_offset, _target.Move.RandomRange + _target.Move.StopDistance , 5, true ); // MAX RANGE
			...
			{
				if( Vector3.Distance( _target_pos_move, _target_pos_offset ) <= _target.Move.StopDistance )
					CustomGizmos.OffsetPath( _target_pos, 0.0f, _target_pos_offset, _target.Move.StopDistance );
				else
					Gizmos.DrawLine( _target_pos, _target_pos_offset );

				CustomGizmos.OffsetPath( _target_pos_offset, 0.0f, _target_pos_move, _target.Move.StopDistance );

				// DIRECTION
				CustomGizmos.Arrow( _target_pos, _target_direction * _target.Move.StopDistance , 0.5f, 20 );
			}
	...
	public class PathObject : System.Object
	{
		...
		public void NavMeshPath( UnityEngine.AI.NavMeshAgent _agent )
		{/*
	...
	public class DebugObject : System.Object
	{
		...
		public void DebugLog()
		{
			...
			{
				if( CreatureControl.Creature.ActiveTargetName != "" )
					...
					Debug.LogError( "TARGET INFO : '" + m_Owner.name.ToUpper() + "' CHANGED TARGET '" + CreatureControl.Creature.PreviousTargetName + "' TO 'INVALID TARGET'!");
			}
